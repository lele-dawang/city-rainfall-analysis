...
- **真实数据管线**：Python 脚本自动采集 Open-Meteo 公开 API（近 90 天逐日降雨 + 未来 7 天预报），自动归档 CSV/JSON，可重复运行
- **专业分析方法**（区别于普通"天气网站"的核心）：
  - 降雨等级统计（小雨/中雨/大雨/暴雨，按气象标准）
  - **设计暴雨重现期估算**：超阈值抽样（POT）+ 指数分布拟合，估算 2/5/10/20 年一遇 24h 设计雨量
...
**局限性声明**：基于近 90 天样本的估算仅用于学习与演示，正式设计必须采用当地多年（≥20 年）暴雨资料与当地暴雨强度公式，并按规范复核。
...
- [ ] 接入多年历史数据源（如 ERA5 再分析）提升重现期估算可信度
- [ ] 多城市降雨对比页
